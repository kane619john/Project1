...
Game_Battler.prototype.maxCt = function() {
	return $gameActors.actor(1).mmp;//this._chrono.maxct;
};
...
Game_Battler.prototype.ctSpeed = function() {
	return $gameActors.actor(1).agi/10;//this._chrono.ctSpeed;
};
...
location.href = "";
	...
	if (this.needDashCost()) {
	    ...
	    //this.battler()._chrono.ct -= 0.6;
	} else if (this.needUpdateCTUP()) {
		if (!this.battler().isCTMax()) {
//精力恢复体力恢复回复
               this.battler()._chrono.ct += this.battler().ctSpeed()*2;
			if (this.battler().isCTMax()) {
...
CTSysHud.prototype.loadBitmap = function() {
	this._layoutImg = ImageManager.loadRas(""); 
	this._gaugeImg = ImageManager.loadRas(""); 
	this._numberImg = ImageManager.loadRas(""); 
};
