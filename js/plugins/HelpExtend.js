...
RJO.HE.getDescParams = function(item, type) { //核心
...
RJO.HE.getItemBaseDescParams = function(item, normalcolor) {
	if(item.consumable) item.descParams.push(["", RJO.HE.ItemDescOtherSize, RJO.HE.ItemDescConsumableColor, true]);
}
...
RJO.HE.getEquipBaseDescParams = function(item, normalcolor) {
	...
	//	if(item.etypeId==1 || item.etypeId==5 || item.etypeId==10)
	//item.descParams.push(["熟练度" + ":" + item.params[0] + "%", RJO.HE.ItemDescOtherSize, paramscolor[0], false]);
	if(item.etypeId<5)item.descParams.push(["强化等级" + ":" + (item.params[1]!=0 ? item.params[1]>9?"顶级":item.params[1] : "无"), RJO.HE.ItemDescOtherSize, paramscolor[1], false]);
	 if(item.params[0] != 0)item.descParams.push([TextManager.param(0) + ":" + item.params[0], RJO.HE.ItemDescOtherSize, paramscolor[0], false]);
	// 	else if (item.etypeId==2 || item.etypeId==3 || item.etypeId==4)
	//	item.descParams.push(["契合度"+":"+item.params[0]+"%",RJO.HE.ItemDescOtherSize,paramscolor[0],false]);
	for(var i = 2; i < item.params.length; i++) {
		value = item.params[i];
		if(value != 0) {
...
RJO.HE.getItemPriceDescParams = function(item, normalcolor) {
	if(item.note.indexOf('货物')!=-1)
	{
		var v1 = ((item.successRate-1)*100).toFixed(2);
		var pt1 = "当前涨跌：" + (v1 >= 0 ? "+" : "") + v1 + "%";
		//if(item.id == 223)alert(item.successRate);
		var v2 = ((1-(item.price/item.speed))*100);
		var pt2 = "至今涨跌："+ (item.speed<=item.price ? "+":"-") + Math.abs(v2).toFixed(2) +"%";
		var pt3 = "价格参考：" + parseInt(item.price);
		var pt4 = "买入价格：" + parseInt(item.animationId);
     	item.descParams.push([pt1, RJO.HE.ItemDescOtherSize,v1 >= 0 ?RJO.IQ.QualitiesColor[5] : RJO.IQ.QualitiesColor[1], false]);
		item.descParams.push([pt2, RJO.HE.ItemDescOtherSize,item.speed<=item.price ?RJO.IQ.QualitiesColor[5] : RJO.IQ.QualitiesColor[1], false]);
		item.descParams.push([pt3, RJO.HE.ItemDescOtherSize, RJO.HE.ItemDescPriceColor, false]);
	if(item.animationId>0)item.descParams.push([pt4, RJO.HE.ItemDescOtherSize, RJO.HE.ItemDescPriceColor, false]);
	}else{ 
	if(item.price > 0) {
		var pt1 = "价格参考：" + Math.round(item.price);
...
Sprite_ItemHelp.prototype.updatePos = function(wx, wy, rect) {
	var sx = wx + rect.x + rect.width - 184;
	var sy = wy + rect.y + rect.height + 14;
