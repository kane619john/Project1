...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_ChronoEnemyHP');
    Moghunter.chrono_enemyHP_layout_x =0
// Number(Moghunter.parameters['Layout X-Axis'] || 0);
    Moghunter.chrono_enemyHP_layout_y =0
//Number(Moghunter.parameters['Layout Y-Axis'] || 400);
...
Game_Action.prototype.afterApplyCN = function(target,result,oldhp,oldmp) {
	...
	if (this.chronoEnemyHPNeedData(target)) {
		...
		$gameTemp._chronoEnemyHP[1] = target._name+"(Cl:"+parseInt(target.mhp/200)+")";//显示敌人的名字 
...
Game_Action.prototype.chronoEnemyHPNeedData = function(target) {
	//DataManager.alertobj(this.subject());
	if (this.subject()._name!=$gameActors.actor(1).name() && this.subject().isEnemy()) {return false};
...
CNHPGauge.prototype.initialize = function() {
	...
	this._layout.x = Moghunter.chrono_enemyHP_layout_x+$enemyhp_X;
	this._layout.y = Moghunter.chrono_enemyHP_layout_y+$enemyhp_Y;  
	this.addChild(this._layout);
	this._hpImage = ImageManager.loadRas("EnemyHP_B");
	this._meter2 = new Sprite(this._hpImage);
	this._meter2.x = this._layout.x + Moghunter.chrono_enemyHP_meter_x+$enemyhp_X;
	this._meter2.y = this._layout.y + Moghunter.chrono_enemyHP_meter_y+$enemyhp_Y;
	this.addChild(this._meter2);		
	this._meter = new Sprite(this._hpImage);
	this._meter.x = this._layout.x + Moghunter.chrono_enemyHP_meter_x+$enemyhp_X;
	this._meter.y = this._layout.y + Moghunter.chrono_enemyHP_meter_y+$enemyhp_Y; 	
	this.addChild(this._meter);
	this._name = new Sprite(new Bitmap(180,32));
	this._name.x = this._layout.x + Moghunter.chrono_enemyHP_name_x+$enemyhp_X;
	this._name.y = this._layout.y + Moghunter.chrono_enemyHP_name_y+$enemyhp_Y;
...
CNHPGauge.prototype.refreshName = function() {
	...
	this._name.bitmap.drawText(this.eName(),0,0,240,32,"left");
};
