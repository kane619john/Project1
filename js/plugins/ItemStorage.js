...
(function() {
    ...
    Game_Party.prototype.setStorageParameters = function(args) {
        ...
        args.forEach(function(a){
            ...
            switch(data[0]) {
                ...
                case 'money' :
                case '所持金':
                    storage.money   = eval(data[1])+Math.randomInt(eval(data[1])/4+1);
                    //随机所持金
                    break;
                case 'buyRate' :
                case '販売レート':
    ...
    Window_Storage.prototype.includes = function(item) {
        ...
        if($gameSwitches.value(5)){//士兵交易
        	if (item.itypeId != 4) return false;
        }
        else if (DataManager.isItem(item) && item.itypeId > 2) return false;
        var key = DataManager.encodeStorageKey(item);
    ...
    //BUG修复
Window_StorageShopBuy.prototype.isCurrentItemEnabled = function() {
    return this.isEnabled(this._data[this.index()]);
};
Window_StorageShopBuy.prototype.isEnabled = function(item) {
    return (item && parseInt(this.price(item)*eval(this._buyRate)) <= this._money &&
...
//物价变动
        price*=1+$gameVariables.value(21)/100;
    ...
    Scene_StorageShop.prototype.createSellWindow = function() {
        ...
        var storage = {name:partyStorageName,storage:$gameParty.allItems(),
            key:this._storage.key,categories:"" };
        //谜之成功
    ...
    Scene_StorageShop.prototype.sellingPrice = function() {
        ...
        price*= 1 + $gameVariables.value(22)/100;
