...
Window_PartyList2.prototype.processTouch = function() {
    if (this.isOpenAndActive()) {
        ...
        if (this._touching) {
            if (TouchInput.isPressed()) {
            	...
            	if(this._touchCount >= 30)this.onTouch(true);
                this._touchCount++;}
            } else {
                this._touching = false;
                if(this._touchCount != -1)this._touchCount = 0;
            }
        }
    } else {
        this._touching = false;
...
//    this._helpWindow.opacity = 255;
	...
	if($gameSwitches.value(11))this._categoryWindow.select(2);
...
Scene_Item.prototype.createListWindow = function() {
    ...
    this._listWindow._touchCount = 0;
...
if(this._listWindow.index!=-1){
	if(this.item().damage.formula.length >= 2){
	if(this.item().name.indexOf('灵药')!=-1)$gameVariables.setValue(691,["耐药",""]);
    this._listWindow.activate();
    this._listWindow.select(-1);
    this._listWindow.show();
    this._detailWindow.show();
	}else {
	$gameVariables.setValue(397,this.item().id);
...
Scene_ItemBase.prototype.itemTargetActors = function() {
	if(this.item().scope == 8) {
		return $gameParty.battleMembers();
	}
    return [$gameActors.actor(this._listWindow.item())]//物品使用对象设计
};
...
Scene_Item.prototype.user = function() {
    ...
    var id = this._listWindow.item();
    if(id>0)return $gameActors.actor(id);
...
Window_ItemListM.prototype.itemHeight = function() {
    return 50;
};
//==============================
// * setCategory
...
// * Includes
//==============================
Window_ItemListM.prototype.includes = function(item) {//宝物类型
	if(item && item.note.indexOf('hide')!=-1)return false;//不可卖店
	if(item && $gameParty.numItems(item)< 0 )return false;
    switch (this._category) {
    case 'item':
        return (DataManager.isItem(item) && item.itypeId === 1) || (DataManager.isArmor(item) && item.etypeId == 7);
    case 'weapon':
        return DataManager.isWeapon(item) || (DataManager.isArmor(item) && item.etypeId != 7);
    case 'armor':
...
Window_ItemListM.prototype.drawItem = function(index) {
    ...
    if (item) {
        ...
        this.drawItemName(item, rect.x, rect.y+5, rect.width - numberWidth);
        this.drawItemNumber(item, rect.x, rect.y+5, rect.width);
...
Window_ItemListM.prototype.drawItemNumber = function(item, x, y, width) {
    if (this.needsNumber() && $gameParty.numItems(item)>1) {
        //this.drawText(':', x, y, width - this.textWidth('00'), 'right');
        this.drawText($gameParty.numItems(item), x-100, y, width, 'right');
    }
};
