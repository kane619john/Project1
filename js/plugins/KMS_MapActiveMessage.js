 ...
 */
//对话文本台词
Scene_Map.prototype.Chatxt = function(str){
	var rand = 0;
	rand = 1;//Math.randomInt(7)+1;
	var arr = [];
	if(rand == 1){
	arr = ["切他中路啊！",
	"厉害了",
	...
	if($gameVariables.value(3) % 15 == 0)arr=["庆典之日快乐！"];
	if($gameVariables.value(204)==5){arr[1]="你说下雨天人们会休战吗？";arr[6]="这雨什么时候能停？"}
	}
	if(rand == 3){
	arr = ["总有一天我也要\n做出一番大事业。",
	"赚钱的方式有很多,\n但我好像一个都做不成。",
	...
	if($gameVariables.value(3) % 15 == 0)arr=["今天可以在酒馆\n一起做美食。"];
	}
	if(rand == 4){
	arr = ["我有很多认识的人都去参了军,\n但却很少见到有回来的……",
	"我有时会想象,\n没有战争的和平日子是怎样的。",
	...
	if($gameVariables.value(529)>15){
		arr[1]="现在好像和平一些了。";
		arr[2]="当今天下有统一之势。"
	}
	if($gameVariables.value(3) % 15 == 0)arr=["希望我还能见到下一次庆典。"];
	}
	...
	if($gameVariables.value(442)==1){
		arr[3]="听说帝国的战士会\n把敌人的头盖骨当碗使。";
		arr[4]="圣咏教国的真神……\n开什么玩笑。";
	}
	if($gameVariables.value(442)==2){
		arr[3]="我永远无法理解\n苍境王国的自然保护法。";
		arr[4]="圣咏教国的真神……\n开什么玩笑。";
	}
	if($gameVariables.value(442)==3){
		arr[3]="我永远无法理解\n苍境王国的自然保护法。";
		arr[4]="听说帝国的战士会\n把敌人的头盖骨当碗使。";
	}
	if($gameVariables.value(581)<0)
	arr[0]="你知道吗,苍境王国正在悬赏\n"+$gameActors.actor(1).nameNick()+ $gameActors.actor(1)._name + ",价值"+ $gameVariables.value(581)*-500 +"G呢。";
	if($gameVariables.value(582)<0)
	arr[1]="你知道吗,司夜帝国正在悬赏\n"+$gameActors.actor(1).nameNick()+ $gameActors.actor(1)._name + ",价值"+ $gameVariables.value(582)*-500 +"G呢。";
	if($gameVariables.value(583)<0)
	arr[2]="你知道吗,圣咏教国正在悬赏\n"+$gameActors.actor(1).nameNick()+ $gameActors.actor(1)._name + ",价值"+ $gameVariables.value(583)*-500 +"G呢。";
	if($gameVariables.value(3) % 15 == 0)arr=["庆典之日的美食\n一定要自己做。"];
	}
	...
	arr[0]="你知道吗,最近出现了\n一个叫"+$gameVariables.value(419)+"的新势力！";
	if($gameVariables.value(529)>3)
	arr[0]="最近"+$gameVariables.value(419)+"似乎发展的很快";
	if($gameVariables.value(529)>10)
	arr[0]="时代真是变了,现在\n"+$gameVariables.value(419)+"如日中天。";
	if($gameVariables.value(106)>=1000){
	arr[1]="最近都在讨论"+ $gameActors.actor(1).nameNick()+ $gameActors.actor(1)._name +"\n我看也没什么了不起的。";
	if($gameVariables.value(106)>=5000)arr[1]="最近"+$gameActors.actor(1).nameNick()+ $gameActors.actor(1)._name +"特别出名。";
	if($gameVariables.value(106)>=10000)arr[1]="你知道"+$gameActors.actor(1).nameNick()+ $gameActors.actor(1)._name +"吗？\n那简直是个传奇。";
	}
	if($gameVariables.value(107)>=10)
	arr[2]="最近的人们都在称赞\n"+$gameActors.actor(1).nameNick()+$gameActors.actor(1)._name+",确实不错。";
	if($gameVariables.value(107)>=15)
	arr[2]="你知道"+$gameActors.actor(1).nameNick()+$gameActors.actor(1)._name+"吗？\n那是个大侠！";
	if($gameVariables.value(107)<=-10)
	arr[2]="最近的人们都在说\n"+$gameActors.actor(1).nameNick()+$gameActors.actor(1)._name+",那是个十足的恶人";
	if($gameVariables.value(107)<=-15)
	arr[2]="你知道"+$gameActors.actor(1).nameNick()+$gameActors.actor(1)._name+"吗？\n那是个恶鬼！遇到千万要小心！";
	
	if($gameVariables.value(3)<=30)
	{
		arr.push("帝国的王位交替……\n这会是战争的导火线吗？");
		arr.push("大坑洞里没有美好的财宝,\n只有吃人的怪物。");
	}
	if($gameVariables.value(668)>=3){
		arr.push("听说失传的符文工艺\n再度出现在了这个世上。");
	}
	if($gameVariables.value(614)>=5){
		arr.push("说时迟,那时快！"+$gameActors.actor(1)._name+"仿佛天下无双,\n一个回合便制服了那敌将！");
	}
	...
	if(rand == 7){
		arr = ["最近没什么特别的。"];
		//if(Math.randomInt(4) == 1)arr[0]="";
		if($gameSwitches.value(421))arr[0]="听说了吗！最近帝国\n率大军在攻打萨班纳！";
		if($gameSwitches.value(422))arr[0]="真的吗？听说苍境王国率军\n誓要夺回萨班纳？";
		if($gameSwitches.value(423))arr[0]="听说了吗！最近帝国\n率大军在攻打伊尔瑞姆！";
		if($gameSwitches.value(424))arr[0]="我就知道,教国正打算率军\n夺回伊尔瑞姆！";
		if($gameSwitches.value(425))arr[0]="听说教国的惩戒大军\n在最近对拿波进行圣战。";
		if($gameSwitches.value(426))arr[0]="我就知道,教国正打算率军\n夺回乌伦！";
		if($gameSwitches.value(427))arr[0]="真的吗？听说苍境王国率军\n誓要夺回拿波？";
		if($gameSwitches.value(428))arr[0]="苍境王国的尖枪\n终于刺向了乌伦。";
		if($gameSwitches.value(429) || $gameSwitches.value(430) || $gameSwitches.value(431)){
			if($gameVariables.value(647)==1)arr[0]="听说最近贼匪\n越来越猖狂了。";
			if($gameVariables.value(647)==2)arr[0]="最近好像出现了一批不死军队……\n这也太恐怖了。";
			if($gameVariables.value(647)==3)arr[0]="最近又出现了一大群魔物,\n早知如此不如把它们永远埋在地下.";
		}
		if($gameSwitches.value(433))arr[0]="难以置信！听说苍境的精灵引发了大海啸,\n把拿波和萨班纳的敌军全赶走了！";
		if($gameSwitches.value(435))arr[0]="帝国最近进行了大征召！\n无数的年轻人都涌入了战场……";
	}
	var txt = arr[Math.randomInt(arr.length)];
	if(str.indexOf("喵")!=-1)txt = txt.slice(0,txt.length-1) + '喵' + txt.slice(txt.length-1);
	return txt;
}
//
Scene_Map.prototype.displayActiveMessage = function()
{
    ...
    while (!isNullOrUndefined(message))
    {
       ...
       if(message.text.indexOf('闲聊')!=-1) message.text = this.Chatxt(message.text);
        {
