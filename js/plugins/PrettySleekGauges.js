...
Special_Gauge.prototype.setRate = function(rate) {
	...
	this._maxRate = rate;
	this._speedRate = Math.abs(this._curRate - this._maxRate) / 10;//动画时间
}
...
Special_Gauge.prototype.setExtra = function(text, val, max, yOffset) {
	...
	if (val || val === 0) {
		if (val != this._setVal) {
			this._speed = Math.max(0, Math.abs(this._curVal - val) / 10);//动画时间
		}
		this._setVal = val;
...
Window_EnemyHPBars.prototype.drawActorTp = function(actor, x, y, width) {
	...
	this._gauges[this.makeTGaugeKey(x + tinyGaugeXOffset, y + 1 + tinyGaugeYOffset)].setExtra(TextManager.tpA, actor.tp,100);
}
