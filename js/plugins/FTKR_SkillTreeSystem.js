...
(function() {
	...
	Game_Actor.prototype.canStsLearnedSkill = function(skillId) {
		...
		return $dataSkills[skillId] && results.filter(function(elm) {
			return !elm;
		});
	};
	...
	Game_Actor.prototype.stsLearnSkill = function(skillId) {
		...
		//属性
		if(skillId==202){
         $gameParty.gainItem($dataArmors[31],1, false);
		}//固若
		if(skillId==208){
         $gameParty.gainItem($dataArmors[21],1, false);
		}//复苏
		if(skillId==214){
         $gameParty.gainItem($dataArmors[32],1, false);
		}//用兵		
		if(skillId==220){
         $gameParty.gainItem($dataArmors[34],1, false);
		}//时停
		if(skillId==222){
         $gameParty.gainItem($dataArmors[22],1, false);
		}//核爆
		if(skillId==226){
         $gameParty.gainItem($dataArmors[13],1, false);
		}//剑舞
		if(skillId==228){
         $gameParty.gainItem($dataArmors[14],1, false);
		}//解放
		if(skillId==232){
         $gameParty.gainItem($dataArmors[15],1, false);
		}//连突
		if(skillId==234){
         $gameParty.gainItem($dataArmors[16],1, false);
		}//超凡连突
		if(skillId==238){
         $gameParty.gainItem($dataArmors[17],1, false);
		}//穿云
		if(skillId==240){
         $gameParty.gainItem($dataArmors[18],1, false);
		}//万箭
		if(skillId==244){
         $gameParty.gainItem($dataArmors[19],1, false);
		}//蓄力
		if(skillId==246){
         $gameParty.gainItem($dataArmors[20],1, false);
		}//剑气
		if(skillId==250){
         $gameParty.gainItem($dataArmors[23],1, false);
		}//炎爆		
		if(skillId==252){
         $gameParty.gainItem($dataArmors[24],1, false);
		}//陨石
		if(skillId==256){
         $gameParty.gainItem($dataArmors[25],1, false);
		}//冰棺
		if(skillId==258){
         $gameParty.gainItem($dataArmors[26],1, false);
		}//零度	
		if(skillId==262){
         $gameParty.gainItem($dataArmors[27],1, false);
		}//召唤
		if(skillId==264){
         $gameParty.gainItem($dataArmors[28],1, false);
		}//黑洞
		if(skillId==268){
         $gameParty.gainItem($dataArmors[29],1, false);
		}//风庇
		if(skillId==270){
         $gameParty.gainItem($dataArmors[30],1, false);
		}//天雷
	  
        switch(skillId) {
			//武力
        	case 168:$gameActors.actor(1).addParam(2,1);break;//强击+1
        	case 169:$gameActors.actor(1).addParam(2,2);break;//强击+2
        	case 170:$gameActors.actor(1).addParam(2,3);break;//强击+3
        	case 171:$gameActors.actor(1).addParam(2,4);break;//强击+4
        	case 172:$gameActors.actor(1).addParam(2,5);break;//强击+5
        	
        	//智力
        	case 178:$gameActors.actor(1).addParam(4,1);break;//智力+1
        	case 179:$gameActors.actor(1).addParam(4,2);break;//智力+2
        	case 180:$gameActors.actor(1).addParam(4,3);break;//智力+3
        	case 181:$gameActors.actor(1).addParam(4,4);break;//智力+4
        	case 182:$gameActors.actor(1).addParam(4,5);break;//智力+5
            //统率
			case 193:$gameActors.actor(1).addParam(7,1);break; //统率+1
        	case 194:$gameActors.actor(1).addParam(7,2);break;//统率+2
        	case 195:$gameActors.actor(1).addParam(7,3);break;//统率+3
        	case 196:$gameActors.actor(1).addParam(7,4);break;//统率+4
        	case 197:$gameActors.actor(1).addParam(7,5);break;//统率+5

			//忍耐
        	case 173:$gameActors.actor(1).addParam(3,1);break;//护甲+1
        	case 174:$gameActors.actor(1).addParam(3,1);$gameActors.actor(1).addParam(0,50);break;//护甲+2
        	case 175:$gameActors.actor(1).addParam(3,2);$gameActors.actor(1).addParam(0,50);break;
        	case 176:$gameActors.actor(1).addParam(3,2);$gameActors.actor(1).addParam(0,50);break;
        	case 177:$gameActors.actor(1).addParam(3,2);$gameActors.actor(1).addParam(0,100);break;
			//护体
        	case 183:$gameActors.actor(1).addParam(5,1);break;//魔防+1
        	case 184:$gameActors.actor(1).addParam(5,2);break;//魔防+2
        	case 185:$gameActors.actor(1).addParam(5,1);break;//魔防+3
        	case 186:$gameActors.actor(1).addParam(5,2);break;//魔防+3
        	case 187:$gameActors.actor(1).addParam(5,2);break;//魔防+3
        	//体操
			case 188:$gameActors.actor(1).addParam(6,1);break; //灵巧
        	case 189:$gameActors.actor(1).addParam(6,2);break;//灵巧
        	case 190:$gameActors.actor(1).addParam(6,3);break;//灵巧
        	case 191:$gameActors.actor(1).addParam(6,4);break;//灵巧
        	case 192:$gameActors.actor(1).addParam(6,5);break;//灵巧
        }
	...
	Window_SkillTree.prototype.makeItemList = function() {
		//this._data = [1001,1004,1006,1007];
		//alert(this._tTypeId);
		var actor = this._actor;
	...
	Window_SkillTree.prototype.update = function() { //按键绑定修改
		Window_Selectable.prototype.update.call(this);
		...
		/*
			if(Input.isTriggered("1")) {
				if($gameVariables.value(381)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(381, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(381, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
			if(Input.isTriggered("2")) {
				if($gameVariables.value(382)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(382, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(382, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
			if(Input.isTriggered("3")) {
				if($gameVariables.value(383)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(383, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(383, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
		if(Input.isTriggered("4")) {
				if($gameVariables.value(384)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(384, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(384, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
		if(Input.isTriggered("5")) {
				if($gameVariables.value(385)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(385, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(385, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
		if(Input.isTriggered("6")) {
				if($gameVariables.value(386)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(386, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(386, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
		if(Input.isTriggered("7")) {
				if($gameVariables.value(387)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(387, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(387, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
		if(Input.isTriggered("8")) {
				if($gameVariables.value(388)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(388, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(388, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
		if(Input.isTriggered("9")) {
				if($gameVariables.value(389)==this._skillId) {
					SoundManager.playCancel();
					$gameVariables.setValue(389, 0);
				} else if($gameActors.actor(1).isLearnedSkill(this._skillId)) {
					$gamePlayer.deskill(this._skillId)
					SoundManager.playEquip();
					$gameVariables.setValue(389, this._skillId);
				} else SoundManager.playBuzzer();
				this._stsStatusTitleWindow.refresh();
		 }
		 */
    ...
    Game_Player.prototype.skillisband = function(id) {//技能是否绑定
    	var band = 0;
        if (id==$gameVariables.value(381))return band=1;
        if (id==$gameVariables.value(382))return band=2;
        if (id==$gameVariables.value(383))return band=3;
        if (id==$gameVariables.value(384))return band=4;
        if (id==$gameVariables.value(385))return band=5;
        if (id==$gameVariables.value(386))return band=6;
        if (id==$gameVariables.value(387))return band=7;
        if (id==$gameVariables.value(388))return band=8;
        if (id==$gameVariables.value(389))return band=9;
        return band;
	};
    Game_Player.prototype.deskill = function(id) {//技能是否绑定
    	//for (var i=0;i<9;i++) {
        //alert($gameVariables.value(381)+"-"+$gameVariables.value(382));
        if (id==$gameVariables.value(381))$gameVariables.setValue(381,0);
        if (id==$gameVariables.value(382))$gameVariables.setValue(382,0);
        if (id==$gameVariables.value(383))$gameVariables.setValue(383,0);
        if (id==$gameVariables.value(384))$gameVariables.setValue(384,0);
        if (id==$gameVariables.value(385))$gameVariables.setValue(385,0);
        if (id==$gameVariables.value(386))$gameVariables.setValue(386,0);
        if (id==$gameVariables.value(387))$gameVariables.setValue(387,0);
        if (id==$gameVariables.value(388))$gameVariables.setValue(388,0);
        if (id==$gameVariables.value(389))$gameVariables.setValue(389,0);
        //alert($gameVariables.value(381)+"-"+$gameVariables.value(382));
    //	}
	};
	Window_StsSkillStatus.prototype.drawSkillState = function(format) {//技能状态
		if(this._actor && this._skillId) {
			var skill = this._actor.stsSkill(this._skillId);
			var y = this.lineHeight();
			var width = this.width - this.padding * 2;
			var band = 0;
			var s = $dataSkills[this._skillId];
			band = $gamePlayer.skillisband(this._skillId);
			format ? this.drawStsDescTitle(format+(band!=0 ? ''+band+'' : "")+' '+
			(s.speed>0 ? '冷却:'+s.speed+'秒':'')+
			(s.mpCost>0 ? ' mp:'+s.mpCost:'')+
			(s.tpCost>0 ? ' tp:'+s.tpCost:''), 0, 0, width, skill) : y = 0;
	...
	Window_StsConf.prototype.makeItemList = function() {
		this._data = [{
				dicision: true,
				disp: FTKR.STS.conf.okFormat
			},
			{
				dicision: false,
				disp: FTKR.STS.conf.cancelFormat
			}
		];
	};
			...
			if(this._skillId) {
				for(var i = 0; i < 3; i++) {
					...
					if(cost) {
						...
						if(FTKR.STS.sp.hideCost0 && (!cost.value || Number(cost.value) === 0)) continue;
						if(Number(cost.value) === 0) continue;
						if(cost.type == 'gold')
							this.drawStsCost(cost, 0, y * ((i * 1)), width);
						else this.drawStsCost(cost, 0, y * (1 + (i * 1.5)), width);
					}
				}
			}
		}
	...
	Window_StsCost.prototype.drawStsCost = function(cost, x, y, width) {
		...
		//x+ diff y 
		this.drawStsFormatText(num[1], x - iw, y + 30, params, numberWidth, 'right');
	};
	...
	Window_StsPreskill.prototype.drawAllPreskill = function(index) {
		if(this._actor) {
			...
			if($dataSkills[this._skillId].message1.length > 0)
			this.drawStsDescTitle($dataSkills[this._skillId].message1, 0, y, width, skill);
			else oy-=1;
			if($dataSkills[this._skillId].message2.length > 0)
			this.drawStsDescTitle($dataSkills[this._skillId].message2, 0, y*2, width, skill);
			else oy-=1
			}
			if(this._skillId && this._tTypeId) {
				...
				for(var i = 0; i < preskillIds.length; i++) {
					...
					if(preskill) {
						...
						this.drawFormatTextEx(FTKR.STS.preskill.itemFormat, 0, y * (oy + i), [preskill.name], width);
						this.changePaintOpacity(1);
					}
				}
		  }
	...
	Window_StsActorStatus.prototype.refresh = function() {
		...
		if(actor) {
			...
			if(Imported.FTKR_CSS) {
				...
				// this.drawActorName(actor, 60, 0, w - 60);
				this.drawExpInfo(0, 0);
				this.drawCssActorSp(actor, 0, y + 30, w - 60);
			}
		}
	};
	Window_StsActorStatus.prototype.drawExpInfo = function(x, y) {
		...
		this.drawText(value2 + " 经验", x, y + lineHeight, 270, 'left');
	};
	...
	Scene_STS.prototype.create = function() {
		...
		//$gameActors.actor(1).actor().sts.treeTypes = [1001,1004,1006,1007];//update修改
