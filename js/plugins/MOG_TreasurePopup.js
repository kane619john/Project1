...
Game_Interpreter.prototype.checkTreasurePopup = function(type) {
	if($gameSystem._trspupVisible) {
		...
		if(SceneManager._scene.constructor.name === "Scene_Map" && $gameMap._mapId != 132) {
			for(i = 0; i < $gameMap.events().length; i++) {
				var eve = $gameMap.events()[i];
				if(eve && (this._eventId === eve._eventId)) {
					var x = eve.screenX();
					var y = eve.screenY();
					if(x >= 1000 || x <= 200 || y >= 600 || y <= 100) {
...
Game_Party.prototype.checkTreasurePopup = function(type) {
	if ($gameSystem._trspupVisible) {
		if (type > 2) {
		   var amount = this.operateValue(this._params[0], this._params[1], this._params[2]);
		} else {
...
TreasureIcons.prototype.setupNew = function(data) {
	this._item = data[0];
	this._amount = data[1];
    var str = this._amount>=0 ? "+":"" ;
	var name = this._item ? this._item.name + str + this._amount : this._amount;
...
TreasureIcons.prototype.createName = function() {
	this._name = new Sprite(new Bitmap(300,32));
...
TreasureIcons.prototype.refreshName = function() {
	...
	if (typeof(this._item) == "string"){
          name = this._item;
          this._name.bitmap.fontSize = Moghunter.trpopup_fontSize+this._amount ;
          this._name.bitmap.drawText(name,0,0,800,32);
	}else this._name.bitmap.drawText(name,0,0,145,32);
};
