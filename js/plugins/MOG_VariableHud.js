...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_VariableHud');
	...
	for (var i = 0; i < Moghunter.variableHud_Max; i++) {
        ...
        Moghunter.variableHud_FileName[i] = String(Moghunter.parameters['Hud ' + String(i + 1) + ' File Name'] || '');
		...
		Moghunter.variableHud_ShowMax[i] = String(Moghunter.parameters['Hud ' + String(i + 1) + ' Show Max Value'] || 'false');
...
Scene_Map.prototype.createSpriteset = function() {
	...
	//游戏确定检查
       // if (location.href.indexOf("?player=true") == -1)
      //  { }
      //  else {
...
if(this.dataSys().maxValue==665){
var foot = $gamePlayer.popfood();
foot=$gamePlayer.popfood();
return Math.max(foot,0);
}
    if(this.dataSys().maxValue==666)return $gameParty.MaxPop();
    if(this.dataSys().maxValue==664)return $gameVariables.value(3);
    if(this.dataSys().maxValue==663)return 0;
...
VariableHud.prototype.number = function() {
	return this.type() === 0 ? $gameVariables.value(this.variableID()) : this.item();
};
...
if(this.variableID()==1)return $gameParty._gold;	
if(this.variableID()==2)return $gameActors.actor(1).tp;
if(this.variableID()==331)return $gamePlayer.popsum()+"/"+$gameParty.MaxPop();
if(this.variableID()==78)return "FPS:"+parseInt(Graphics._fpsMeter.fps);
var item = $dataItems[this.variableID()];
if(item){
return $gameParty.numItems(item);}else return 0;
};
...
VariableHud.prototype.refreshNumber = function() {	
   ...
   //    if (this.maxValue()){
//	var text = (this._variable)+ "/" + this.maxValue();
    //    }   else 
        var text = (this._variable);
...
VariableHud.prototype.refreshHud = function() {
	if (this.type() == 1) {
		if (this.number() < 0) {
		if(this.variableID()==15)$gameVariables.setValue(this.variableID(),0);
		   // alert(1);}
		};
	};
	if (this.type() == 0) {
		if (this.number() < 0) {
		if(this._maxValue==663)$gameVariables.setValue(this.variableID(),0);
		};
	};
