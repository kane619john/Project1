...
Game_System.prototype.isQuestObjectiveCompleted = function(questId, objId) {
  if (this._questsObjectivesCompleted[questId]) {
  	alert(this._questsObjectivesCompleted[questId].contains(objId));
...
Window_QuestData.prototype.getQuestDescription = function() {
  ...
  //return JSON.parse(questData.description[index]);修改
  return questData.description[index];
};
...
Window_QuestData.prototype.getQuestRewards = function(wordwrap) {
  ...
  for (var i = 0; i < length; ++i) {
    ...
    var fmt = this.settings(key);
   // text += fmt.format(JSON.parse(lineData[rewardId]));修改2
   text += fmt.format(lineData[rewardId]);
  }
  return text;
};
...
Window_QuestCategories.prototype.itemHeight = function() {
    return 60;
};
...
Scene_Menu.prototype.commandQuest = function() {
	//修改
	for(var i = 1; i <= 5; i++) {
		if($gameVariables.value(170 + i) != 0) {
			var job = $gameVariables.value(170+i);
			$dataQuests[i].name = job.name;
			$dataQuests[i].difficulty = job.dif;
			$dataQuests[i].description[1] = job.job;
			if(job.from != null) $dataQuests[i].from = job.from;
			else $dataQuests[i].from = "Error"
			if($gameVariables.value(170+i).locat <= 0)$dataQuests[i].location = "已完成";
			else $dataQuests[i].location = job.locatname + job.locat;
			if(job.gitem != null)
				$dataQuests[i].rewards[1] = job.gold + "金钱  " + $dataItems[job.gitem].name;
			else $dataQuests[i].rewards[1] = job.gold + "金钱";
		}
		else $dataQuests[i].location = "未领取";
	}
