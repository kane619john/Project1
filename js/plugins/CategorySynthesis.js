...
Imported['CategorySythesis'] = 1.01;

/*:
 * @plugindesc ver1.01/アイテムを合成する機能を追加します。【SecondaryCategories必須】
 * @author Yana
 *
...
*/
    ...
    Window_Recipe.prototype.isEnabled = function(item) {
        ...
        //if(!DataManager.isItem(item) && $gameParty.maxItems(item)==1 && $gameParty.hasItem(item,true))return false;
        return result;
    };
    ...
    Window_Material.prototype.refresh = function() {
        ...
        if (this._category) {
            ...
            if (this._price && this._number) {
                ...
                this.changeTextColor(this.normalColor());
                this.drawText(this._price * this._number, -30, 0, this.contentsWidth() - 36, 'right');
                this.changeTextColor(this.systemColor());
    ...
    Scene_CategorySynthesis.prototype.createGoldWindow = function() {
        ...
        this._goldWindow.x = Graphics.boxWidth - this._goldWindow.width -200;
    ...
    Scene_CategorySynthesis.prototype.createCategoryWindow = function() {
        ...
        var w = Graphics.boxWidth - this._goldWindow.width - 200;
       ...
       // 记录点this._categoryWindow.activate();
    ...
    Scene_CategorySynthesis.prototype.createRecipeListWindow = function() {
        var x = 0;
        var y = this._categoryWindow.y + this._categoryWindow.height+20;
    ...
    Scene_CategorySynthesis.prototype.onRecipeCancel = function() {
        ...
        this._helpWindow.clear();
        SceneManager.push(Scene_Map);//退出
    };
    ...
    Scene_CategorySynthesis.prototype.doSynthesis = function() {
    	...
    	if(this._recipeWindow.item().note.indexOf("decompose")!=-1){//分解
         var item = this._listWindow._sets[0];
         if(item.meta.quality == 5)
         $gameParty.gainItem($dataItems[98], this._estimateWindow.itemNumber()*3);
         if(item.meta.quality == 4)
         $gameParty.gainItem($dataItems[98], this._estimateWindow.itemNumber()*2);
         if(item.meta.quality == 3)
         $gameParty.gainItem($dataItems[98], this._estimateWindow.itemNumber());
         if(item.meta.quality == 2)
         $gameParty.gainItem($dataItems[97], this._estimateWindow.itemNumber()*3);
         if(item.meta.quality == 1)
         $gameParty.gainItem($dataItems[97], this._estimateWindow.itemNumber()*2);    
         if(item.meta.quality == 0)
         $gameParty.gainItem($dataItems[97], this._estimateWindow.itemNumber());    
         
         $gameParty.loseItem(this._listWindow._sets[0], num);
         if (this._recipeWindow.currentPrice()) $gameParty.loseGold(this._recipeWindow.currentPrice() * num);
         this._goldWindow.refresh();   
    	}
    	else if(this._recipeWindow.item().note.indexOf("强化")!=-1){
    	 //强化
    	 var item = this._listWindow._sets[0];
    	 if(Math.randomInt(100)+1 >= item.params[1]*10){
    	 var upbool = $gameVariables.value(520).indexOf(item); 
    	 if(DataManager.isWeapon(item)){
    	 $dataWeapons[item.id].params[1]+=1;
         for (var i=0;i<=7;i++) {
         	if(i!=1 && $dataWeapons[item.id].params[i]>0){
         	if(i==0)$dataWeapons[item.id].params[i]+=Math.min(20,Math.max(10,parseInt($dataWeapons[item.id].params[i]*0.1)));
         	else $dataWeapons[item.id].params[i]+=Math.min(2,Math.max(1,parseInt($dataWeapons[item.id].params[i]*0.1)));
            }
         }
    	 DataManager.processHENote($dataWeapons, 1); 
    	 alert("强化成功");
    	 }
    	 if(DataManager.isArmor(item)){
    	 $dataArmors[item.id].params[1]+=1;
         for (var i=0;i<=7;i++) {
         	if(i!=1 && $dataArmors[item.id].params[i]>0){
         	if(i==0)$dataArmors[item.id].params[i]+=Math.min(10,Math.max(1,parseInt($dataArmors[item.id].params[i]*0.1)));
         	else $dataArmors[item.id].params[i]+=Math.min(2,Math.max(1,parseInt($dataArmors[item.id].params[i]*0.1)));
         	}
         }
    	 DataManager.processHENote($dataArmors, 1);
    	 alert("强化成功");
    	 }
    	 if(upbool != -1)$gameVariables.value(520)[upbool] = item;
    	 else $gameVariables.value(520).push(item);//强化数组记录
    	 
        }else {
        alert("强化失败");
        }
    	for (var i=1,max=this._listWindow._sets.length;i<max;i++) {
            var item = this._listWindow._sets[i];
        ...
        //$gameTemp.registerQuickNotification({text:'合成成功，获得'+this._estimateWindow.item().name+"x"+this._estimateWindow.itemNumber(),faceName:'',faceIndex:''});
