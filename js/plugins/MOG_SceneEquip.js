...
Scene_Equip.prototype.createSlotWindow = function() {
	if( !Moghunter.scEquip_ComWindow_visible ){
		...
		this._slotWindow.select(-1);//修改 选项窗口的绑定事件 到 装备列表中
	}else{
		_mog_scsEquipM_createSlotWindow.call(this);
	}
};
Scene_Equip.prototype.onSlotOk = function() {
	if(this._slotWindow.index()==-1){
	this._slotWindow.activate();
...
Scene_Equip.prototype.popScene = function() {
	$gameTemp.reserveCommonEvent(291);
    SceneManager.pop();
};
...
Window_EquipSlot.prototype.drawItem = function(index) {
    ...
    if (this._actor) {
		...
		if( Moghunter.scEquip_Slot_Name_visible ){
            this.drawText(this.slotName(index), rect.x, rect.y+5, 138, this.lineHeight());
		}
        this.drawItemName(this._actor.equips()[index], rect.x + Moghunter.scEquip_Slot_left_margin, rect.y+5);
...
Window_EquipCommand.prototype.itemRect = function(index) {
    var rect = new Rectangle();
    var maxCols = this.maxCols();
    rect.width = this.contents.width / maxCols;
    rect.height = 50;//this.contents.height ;
    rect.x = index % maxCols * (rect.width + this.spacing()) - this._scrollX;
    rect.y = Math.floor(index / maxCols) * rect.height - this._scrollY;
    return rect;
};
...
Window_EquipStatus.prototype.refreshFaceSprite = function() {
	...
	this._faceSprite.bitmap = ImageManager.loadPicture("");
	 //this._faceSprite.bitmap = ImageManager.loadPicture(this._actor._faceName+this._actor._faceIndex);
};
...
// * Window Equip Item
//==============================
var _mog_scEquip_Wequip_drawItemName = Window_Base.prototype.drawItemName;
Window_EquipItem.prototype.drawItemName = function(item, x, y, width) {
    ...
    _mog_scEquip_Wequip_drawItemName.call(this,item, x, y, width+150)
};
