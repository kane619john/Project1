...
Galv.SPAWN.event = function(eventId,x,y,save) {
	...
	if (y == undefined || y === true) {
        ...
        Galv.SPAWN.clearSSwitches($gameMap._mapId,eventId)
...
Galv.SPAWN.canSpawnOn = function(x,y,regions) {
	...
	//if ($gameMap.eventsXy(x, y).length > 0) return false;                        // No spawning on other events���������¼�֮��
			//if ($gamePlayer.x == x && $gamePlayer.y == y) return false;                  // No spawning on player�������ڽ�ɫ֮��
...
Galv.SPAWN.clearid = function(eid) {
	var mapId = $gameMap._mapId;
	delete($gameMap._savedSpawnedEvents[mapId][eid]);
...
Game_Map.prototype.spawnEvent = function(id,x,y,save) {
	...
	this._events[eId]._oldid = id;//旧ID判定
...
Game_SpawnEvent.prototype.initialize = function(mapId,eventId,x,y,spawnEventId,saveEvent) {
	...
	DataManager.extractMetadata(this.event());
        Galv.SPAWN.clearSSwitches(mapId,eventId);
};
