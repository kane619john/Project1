 ...
 *   只作用于事件，触发事件的独立开关。
 *   单独对鼠标有效。
 * 2.ON触发的范围为事件行走图大小。注意不要在空的行走图中加触发设置。
 * 3.该插件的触发跨事件页。
 * 4.如果你设置了"悬停"+"不在悬停区域时"，一定要确保事件两边都配置了行走图，
 *   不然会不停地 行走图 -> 空行走图 来回切换。
 * 5.rmmv默认右键会进入菜单，你如果需要使用右键，可以在插件"互动-鼠标辅助
 *   操作面板" 里面禁用右键菜单。
 * 6.你可以设置对话框弹出时，鼠标仍然可触发事件。
 *   但是，事件指令仍然会被阻塞，只有对话框结束后才会执行事件的指令。
 * 7.左键按下 + 接近鼠标的移动路线 + 任何位置左键释放 = 拖拽事件效果。
 * 
 * -----------------------------------------------------------------------------
 * ----激活条件 - ON条件
				...
				if (l.code === 108) {
					...
					if (command == "=>鼠标触发"){	//=>鼠标触发 : 悬停 : 触发独立开关 : A
						if(args.length >= 6){
							...
							if( temp1 == "关闭独立开关" ){
								var obj = {};
								obj._event_id = this._character._eventId;
								obj._type = type;
								obj._switch = temp2;
								$gameSystem.drill_MTE_pushData(obj);
							}
