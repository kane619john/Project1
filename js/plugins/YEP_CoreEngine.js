...
Graphics.processErrorStackMessage = function(stack)  {
  ...
  data.unshift('非常抱歉！游戏崩溃或遇到BUG了！！<br>');
  for (var i = 1; i < data.length; ++i) {
    data[i] = data[i].replace(/[\(](.*[\/])/, '(');
  }
  data.push('<br><font color="yellow"><b>请使用F5刷新或者直接重启游戏！' +
    '</b></font><br>');
  return data;
};
...
DataManager.processCORENotetags3 = function(group) {
  for (var n = 1; n < group.length; n++) {
    var obj = group[n];
    var notedata = obj.note.split(/[\r\n]+/);
    //obj.maxLevel = Yanfly.Param.MaxLevel;最大的等级修改
...
Yanfly.updateResolution = function() {
  ...
  if (!Imported.ScreenResolution) {
   ...
   if ((navigator.userAgent.match(/(iPhone|iPod|Android|ios|iOS|iPad|Backerry|WebOS|Symbian|Windows Phone|Phone)/i))) {
       window.resizeBy(window.screen.width,window.screen.height);
    }else{
...
Game_Party.prototype.maxItems = function(item) {
    if (!item) return 1;
    if(item.id>=1 && item.id<=8)return $gameVariables.value(item.id+130);
    return item.maxItem;
};
...
Window_Base.prototype.drawCurrencyValue = function(value, unit, wx, wy, ww) {
    ...
    this.drawText(text, wx+42, wy, ww - cx - 4, 'left');
    if (this.usingGoldIcon(unit)) {
