...
Game_Battler.prototype.loadToolSysNotes = function() {
	 ...
	 if (note.toLowerCase() == "friend"){
			 this._ras.friend = true;
	     };	
	 if (note.toLowerCase() == "noass"){
			 this._ras.ass = false;
	     };	
         var note_data = note.split(' : ')
	     ...
	     if (note_data[0].toLowerCase() == "dead item id"){
			 this._ras.deaditem.push(Number(note_data[1]));
	     };	
...
Game_Actor.prototype.setToolItemID = function() {
	var item = $dataItems[this._toolItemId2];
	if (item) {
...
Game_Actor.prototype.setToolSkillID = function() {
	var item = $dataItems[this._toolSkillId];
	if (item) {
...
//重新载入
Game_Map.prototype.getToolsData2 = function() {
	var tempEvent = []
	$gameSystem._toolsData = [];
	var mapId = Number(Moghunter.toolSys_MapID);
	if (!$dataMapTool || !$dataMapTool.events) {return};
	for (var i = 0; i < $dataMapTool.events.length; i++) {
	      if ($dataMapTool.events[i]) {
			  tempEvent[i] = new ToolEvent(mapId,i,$gameMap._events[78],true);
			  $gameSystem._toolsData.push(tempEvent[i])
			  tempEvent[i] = null
		  };
	};	
...
//伤亡判定
Game_Interpreter.prototype.Deads = function(arr1,arr2) {
...
Game_CharacterBase.prototype.setDeadEnemy = function(char,battler) {
   ...
   if (!char._user.treasure[0]) {
    ...
    if(char._oldid != 0){
    //双方伤亡
    if ($gamePlayer.isOpponent($gamePlayer.battler(),char.battler())){
    if(char.event().name.indexOf("BOSS")!=-1){
    if($gameVariables.value(354).indexOf(char._oldid)==-1)$gameVariables.value(354).push(char._oldid);
    }else $gameVariables.value(354).push(char._oldid);}//敌人
    else {
    if (char.event().name.indexOf("角色")!=-1 && char.event().name.indexOf("人偶")==-1){
    $gameActors.actor(parseInt(char.event().note)).setHp(1);	
    if(char.event().name.contains("玩家")){$gameTemp.reserveCommonEvent(7)}
    else {$gameVariables.setValue(212,parseInt(char.event().note)),$gameSwitches.setValue(93,true);};
    $gameVariables.setValue(301,$gameVariables.value(301)+1)}
    if (char.event().note == "援军"){
    if(char.event().name.indexOf("boss")!=-1){
    if($gameVariables.value(355).indexOf(char._oldid)==-1)$gameVariables.value(355).push(char._oldid);
    }else $gameVariables.value(355).push(char._oldid);}//援军
    } }
...
Game_CharacterBase.prototype.setDeadItem = function(battler) {
	for (var i = 0; i < battler._ras.deaditem.length; i++) {
		var variableId = battler._ras.deaditem[i];
		...
		if (variableId) {
			...
			//死亡减少道具
			//规模判定
//if($gameVariables.value(18).length >= 100)num=2;
//if($gameVariables.value(18).length >= 300)num=3;
//if($gameVariables.value(18).length >= 500)num=4+Math.randomInt(2);
//if($gameVariables.value(18).length >= 1000)num=5+Math.randomInt(6);
//if($gameVariables.value(18).length >= 5000)num=15+Math.randomInt(10);
//if(variableId==208)num=1;
$gameParty.loseItem($dataItems[variableId],num)
		};
	};
};
...
Game_Player.prototype.commandGuardUsable = function() {
	...
	if (Input.isPressed(Moghunter.ras_buttonGuard)) {return true};
        if ($gameSwitches.value(101)==true){return true};
    return false;
};
...
//
//NAME
Game_Event.prototype.newname = function() {
	var name1 = ["卡", "龙", "杰", "古", "安", "马", "张", "绝", "瑞", "耶", "飞", "亚",
		"莫", "福", "诺", "帕", "爱", "赵", "米", "慕容", "欧阳", "牛", "Mr.", "六", "乔", "迪", "喵",
		"歌", "卫", "塞", "王", "圣","齐","雷","奥","利","凌","娜","杨","老"
	];
	var name2 = ["俄斯", "西", "冬", "贝", "楠", "汪", "喵", "尔", "斯基", "米", "罗亚", "葛", "飞", "斯",
		"恩", "密", "丽丝", "怕", "心","傲", "贝斯", "鸣", "希", "童", "克斯", "乔", "太郎", "亚", "奥", "兹", "波罗"
		,"天", "景", "明", "铭","云", "罗","小" ,"达", "利","姆", "欧", "菊", "斯达", "乔治", "德华", "白", "莫斯"
		,"克斯","柯丝","里斯","露","克","娜","过","三","丰","佬"
	]

	var namep1 = ["暴风", "火焰", "光明", "月亮", "革命", "混沌", "百万", "愚人", "明日", "疯狂", "虐猫", "食人", "影子",
		"爱猫", "爱狗", "最强", "灾祸", "偶像", "冷酷", "残忍", "雷霆", "冰冷", "血","至高",
		"白金", "奇迹", "龙", "狼", "堕落", "呻吟", "审判", "破灭", "彷徨",
		"黄金","性感","钢铁","魔术","黑暗","暗影","第一","银色","黄色","猎魔","恶魔","神","绿色","米达兰顿的",
		"萨班纳的","乌伦的","布哈拉顿的","塞克纳的","里昂的","赛瑞尔的","萨洛纳斯的","加姆的","伊尔瑞姆的","乌里苏亚的"
	]
	var namep2 = ["战士", "化身", "骑士", "狂人", "武者", "领主", "星", "先知", "强者", "专家", "王", "钻石",
		"诗人", "使者", "大人", "妖精", "伯爵", "圣人", "大公", "猎犬", "剑", "战车", "佣兵", "怪物",
		"屠夫","名将","闪光","之手","仙人","真人","精英"
	]
	var n = namep1[Math.randomInt(namep1.length)] + namep2[Math.randomInt(namep2.length)];
	var tn = name1[Math.randomInt(name1.length)] + name2[Math.randomInt(name2.length)];
	if(Math.randomInt(3) == 0) tn += name2[Math.randomInt(name2.length)];
	return "【"+n+"】"+tn;
}
//emmm
var enemyName_initMembers = Game_Enemy.prototype.initMembers;
Game_Enemy.prototype.initMembers = function() {
	enemyName_initMembers.call(this);
	this._name = "";
};
//==============================
// * check Tool Comments
...
Game_Event.prototype.checkToolComments = function() {
	...
	if (!this._erased && this.page()) {this.list().forEach(function(l) {
	       if (l.code === 108) {var comment = l.parameters[0].split(' : ')
				...
				if (!this._tool.enabled && comment[0].toLowerCase().indexOf("enemy") != -1){
				    this._user.hookshotEvent = true;
				};
				...
				if (comment[0].toLowerCase() == "enemy_id"){
					...
					if ($dataEnemies[enemyId]) {//生成敌人创建敌人
				       if (!this._user.battler) {
				       	if(enemyId == 173 || enemyId >= 175){
				       		var a = $gameActors.actor(parseInt(this.event().note));
				       		$dataEnemies[enemyId].traits = a.allTraits();
				       		$dataEnemies[enemyId].name = a.name();
				       		$dataEnemies[enemyId].params[0] = a.paramBase(0);
				       		$dataEnemies[enemyId].params[1] = a.paramBase(1);
				       		$dataEnemies[enemyId].params[2] = a.paramBase(2);
				       		$dataEnemies[enemyId].params[3] = a.paramBase(3);
				       		$dataEnemies[enemyId].params[4] = a.paramBase(4);
				       		$dataEnemies[enemyId].params[5] = a.paramBase(5);
				       		$dataEnemies[enemyId].params[6] = a.paramBase(6);
				       		//$dataEnemies[enemyId].params[7] = a.paramBase(7);
				       	}
				       	this._user.battler = new Game_Enemy(enemyId, 0, 0);
				       	this._user.battler._name = this._user.battler.enemy().name;
				       	if($gameMap._mapId==175 && enemyId>=175)this._user.battler._ras.friend=false;
				       	if(this._user.battler.enemy().note.indexOf("actor") != -1){		       		       		
				       		this._user.battler._paramPlus = [a.paramPlus(0),a.paramPlus(1),a.paramPlus(2),
				       		a.paramPlus(3),a.paramPlus(4),a.paramPlus(5),a.paramPlus(6),0];
				         	this._user.battler._name = a._name;	
				       		this._user.battler._hp = a._hp;
				       		this._user.battler._mp = a.actorId();
				       		this._user.battler._tp = a.fdr*100;
				       		this._user._aid = a.attackAnimationId1();
				       		this._user.battler.refresh();
				       	}
				       	else {
                       	if(this._user.battler.enemy().note.indexOf("friend") != -1 && this._user.battler.enemy().note.indexOf("actor") == -1){
                        var rate = $gameVariables.value(588)*0.1 + $gameActors.actor(1).exr*0.01;
				       	this._user.battler._paramPlus = [this._user.battler.mhp*rate,0,this._user.battler.atk*rate,this._user.battler.def*rate,this._user.battler.mat*rate,this._user.battler.mdf*rate,this._user.battler.agi*rate,0];
				       	if(this._user.battler._paramPlus[6]>=700)this._user.battler._paramPlus[6]=700;
                        this._user.battler.gainHp(99999);}
                       	if($gameVariables.value(590)!=0 && this._user.battler.enemy().note.indexOf("friend") == -1){
                        var rate = $gameVariables.value(590)*0.1;
				       	this._user.battler._paramPlus = [this._user.battler.mhp*rate,0,this._user.battler.atk*rate,this._user.battler.def*rate,this._user.battler.mat*rate,this._user.battler.mdf*rate,this._user.battler.agi*rate,0];     
				       	if(this._user.battler._paramPlus[6]>=700)this._user.battler._paramPlus[6]=700;
                        this._user.battler.gainHp(99999);}
                       }
...
// * on States Turn状态判断
//==============================
Game_Chrono.prototype.onStatesTurn = function(battler) {
...
if(!$gameSwitches.value(112) && this.battler()._hp > 0) {
	this._Turn[0] += 1;
	this._Turn[1] += 1;
	if(this._Turn[0] >= 60) {
		this.battler().gainHp(this.battler().hrg * 20);
		if(this.battler()._ras.friend) {
			$gameVariables.setValue(535, $gameVariables.value(535) + this.battler().hrg * 20);
			if(this.battler().isEnemy() && this.battler().enemy().note.contains('actor')) $gameActors.actor(this.battler().mp)._param_g[5][0] += this.battler().hrg * 20;
		}
		this._Turn[0] = 0;
		if(this.event().name.indexOf('角色') != -1 && this.event().name.indexOf("人偶") == -1) {
			if(this.battler()._hp > 0)$gameActors.actor(this.battler()._mp).setHp(this.battler()._hp);
			$gameActors.actor(this.battler()._mp).setTp(this.battler()._tp);
		}
	}
	if(this._Turn[1] >= 60) {
		if(this.battler().trg * 100 < 15)this.battler().gainTp(this.battler().trg * 100);
		else this.battler().gainTp(15);
		this._Turn[1] = 0;
	}
}
		/*
		if(this._Turn[0]>= 60/Math.abs(this.battler().hrg*20)){
			if(this.battler().hrg>0)this.battler().gainHp(1);else this.battler().gainHp(-1);
			if(this.battler()._ras.friend){$gameVariables.setValue(535,$gameVariables.value(535)+1);
			if(this.battler().isEnemy() && this.battler().enemy().note.contains('actor'))$gameActors.actor(this.battler().mp)._param_g[5][0]+=1;}
			this._Turn[0] = 0;
		}
		if(this._Turn[1]>= 60/Math.abs(this.battler().trg*100)){
			if(this.battler().trg>0)this.battler().gainTp(1);else this.battler().gainTp(-1);
			this._Turn[1] = 0;
		}
		if(this.event().name.indexOf('角色')!= -1 && this.event().name.indexOf("人偶")==-1){
        $gameActors.actor(this.battler()._mp).setHp(this.battler()._hp);
        $gameActors.actor(this.battler()._mp).setTp(this.battler()._tp);
		}
	}
*/
...
Game_Action.prototype.applyCN = function(target,coopUsers) {
    ...
    //if(result.evaded && target.isEnemy() && target.enemy().note.contains('actor'))$gameActors.actor(target.mp)._param_g[3][0]+=1;//闪避
    result.physical = this.isPhysical();
    result.drain = (this.subject().stateRate(144) > 1);
    if (result.isHitCR()) {//吸血显示,伤害显示
...
ToolEvent.prototype.initialize = function(mapId, eventId , user,getdata) {
	...
	if (this._tool.user && this._tool.user.battler()) {
        ...
        if (this._tool.user.battler() != null){
...
ToolEvent.prototype.checkToolNotes = function() {
	if (!this._erased && this.page()) {this.list().forEach(function(l) {		
	       if (l.code === 108) {var comment = l.parameters[0].split(' : ')	
				...
				if (this.user().battler() && this.user().battler().isActor()){
					//防止像素移动导致的位置错乱修正
					var xz = Math.round(this.user()._x)-this.user()._x;
					//alert(xz);
				    if(Math.abs(xz) > 0.5)this._tool.offsetX =-15;
				    else this._tool.offsetX =15;
				    if(Math.abs(xz) < 0.1)this._tool.offsetX =0;
					//alert(this._tool.offsetX);
					
					var yz = Math.round(this.user()._y)-this.user()._y;
				    if(Math.abs(yz) > 0.5)this._tool.offsetY =-15;
				    else this._tool.offsetY =15;
				    if(Math.abs(yz) < 0.1)this._tool.offsetY =0;
				}
...
//==============================
// * Update Tool Duration刷新TOOL TOOL死亡
//==============================
ToolEvent.prototype.updateToolDuration = function() {
	...
	if (this._tool.duration <= 0) {
		...
		$gameSystem._eventDataToolRequestRemoveSprite = true;
                //setTimeout(this._tool = null,2000);
	};		
};
...
ToolEvent.prototype.collidedXY = function(target) {
	if (target.battler()  && target.battler().isActor()){
	 var bodySize = target.battler() ? target.battler()._ras.bodySize : 0;
    var cxy = [Math.round(target._realX),Math.round(target._realY) - bodySize];
    var dx = cxy[0] - this.x;
    var dy = cxy[1] - this.y;
...
ToolEvent.prototype.playHitAnimation = function(target) {
	...
	if(animationID==-1){animationID=1;
	if(this.user()._user._aid != null)animationID=this.user()._user._aid;}  
...
ToolEvent.prototype.collisionGuard = function(char,battler) {
     if (!battler.isStateAffected(20))
      {//格挡值的判断 格挡的精力消耗
      var c_t = this.battler().agi*(-1-(100-$gameVariables.value(65))/100);
      battler._chrono.ct -= Math.abs(c_t);
...
ToolEvent.prototype.canCollideBattler = function(target) {
	...
	if (!this.item()) {return false};
        //防反 this.isGuardingDirection(target)
        if (target.battler().isActor() &&
        target.battler().isStateAffected(20) &&
        !this.isASS(target) && !this.battler().isStateAffected(13)) {
        this.collisionGuard(target,target.battler());
        $gameTemp.reserveCommonEvent(25);
        target.battler().removeState(20);
        return false
        };
        //防反敌对 this.isGuardingDirection(target)
        if (!target.battler().isActor() && !this.isASS(target) &&
        target.battler().isStateAffected(20) && !this.battler().isStateAffected(13)) {
        this.collisionGuard(target,target.battler());
        if(target._eventId==$gameVariables.value(169))$gameSwitches.setValue(125,true);
        var key = [target._mapId,target._eventId,'C'];
        $gameSelfSwitches.setValue(key, true);
        target.battler().removeState(20);
        //if(target.battler().enemy().note.contains('actor'))$gameActors.actor(target.battler().mp)._param_g[3][0]+=1;//格挡次数
        return false
        };
        //防御
        if (target.battler().isStateAffected(21)  &&  
        target.battler()._chrono.ct >= this.battler().agi &&
        !this.battler().isStateAffected(13) ) {
        this.collisionGuard(target,target.battler());
		target.battler().gainSilentTp(2);
		
        return false
        }; 

		//魔法防御
	    if (target.battler().isStateAffected(58)  &&  
        !this.isASS(target) && this.item().hitType==2) {
        this.collisionGuard(target,target.battler());
		//$gameTemp.reserveCommonEvent(25);
		target.battler().gainSilentTp(2);
        return false
        ...
        if (target.battler().isStateAffected(22) && 
        target.battler()._chrono.ct <= this.battler().agi &&
        target.battler().isStateAffected(20)){
        target.battler()._chrono.ct = 0;
        $gameSwitches.setValue(108,false);
        };
        if (!this.isGuardingDirection(target)){
        $gameSwitches.setValue(108,false);
        };
	
        if (this.battler()!=null && target.battler()!=null
        && target.battler()._ras.ass){
	if (target.direction() == this.direction() 
         && !target.battler().isStateAffected(23)){
        // this.battler().addState(23);
         };
        } 


	if (this.targetCanGuard(target,target.battler()) && 
        target.battler()._chrono.ct >= this.battler().agi*10) {
//
	this.collisionGuard(target,target.battler());
		return false;
	};
...
// * isASS
//==============================
ToolEvent.prototype.isASS = function(char) {
	if (char.direction() === this.direction()) {return true};
	return false;
};
//==============================
// * available Targets
...
ToolEvent.prototype.isFriend = function(user,target) {
	...
	{
	if(user._ras.friend == true)return true;
	};
	if (user.isActor() && target.isEnemy()) 
	{
	if(target._ras.friend == true)return true;
	};
	if (user.isEnemy() && target.isEnemy()
        && ((target._ras.friend == false &&
            user._ras.friend == false) || 
            (target._ras.friend == true &&
            user._ras.friend == true)) ) {return true};
...
ToolEvent.prototype.isOpponent = function(user,target) {
	if (user.isEnemy() && target.isActor()) 
	{
	if(user._ras.friend == false)return true;
	};
	if (user.isActor() && target.isEnemy()) 
	{
	if(target._ras.friend == false)return true;
	else  return false;
	};
	if (user.isEnemy() && target.isEnemy())
	{
	if(user._ras.friend == true && target._ras.friend == true)return false;
        else if(user._ras.friend == true || target._ras.friend == true)return true;
	};
	return false;
};
...
ToolEvent.prototype.gainGold = function(char,battler) {
	...
	var x = $gamePlayer.screenX();
    var y = $gamePlayer.screenY();
    $gameSystem._trspupData.push([null,gold,x,y]);
	}
...
ToolCursor.prototype.createCursor = function() {
    this._cursor = new Sprite(ImageManager.loadRas(""));
...
TurnCursor.prototype.initialize = function() {
	...
	this.bitmap = ImageManager.loadRas("");
	this._actor = null;
};
...
Window_ToolList.prototype.drawItemNumber = function(item, x, y, width) {
	var food = parseInt($gameParty.numItems(item)*item.tpGain/10);
	var money = parseInt($gameParty.numItems(item)*item.speed);
	this.drawText('数量:'+$gameParty.numItems(item), x-500, y, width, 'right');
	this.drawText('军粮:'+Math.max(0,food) , x-380, y, width, 'right');
	this.drawText('工资:'+money, x-230, y, width, 'right');
    this.drawText('训练度:'+item.successRate+'%', x-40, y, width, 'right');
...
ChronoWindows.prototype.createCommandWindow = function() {
	...
	this._comImg2 = ImageManager.loadRas("");
	this._comImg3 = ImageManager.loadRas("");
...
Spriteset_Map.prototype.createResultCR = function() {
	this._resultCR = new Sprite(ImageManager.loadRas(""));
...
Spriteset_Map.prototype.createChronoPhase = function() {
	...
	this._phaseCN = ImageManager.loadRas("");
...
Spriteset_Map.prototype.cacheSpriteChronoCache = function() {
	...
	this._chronoSpriteCache.push(ImageManager.loadRas(""));
	this._chronoSpriteCache.push(ImageManager.loadRas(""));
	this._chronoSpriteCache.push(ImageManager.loadRas(""));
	this._chronoSpriteCache.push(ImageManager.loadRas(""));
	this._chronoSpriteCache.push(ImageManager.loadRas(""));
...
Sprite_Character.prototype.setupDamagePopup = function() {
    if (this.battler().isDamagePopupRequested()) {
            ...
            sprite.y = this.y;
             //敌人血条
            if (this.battler().isEnemy() && $gameVariables.value(169)!=0){
            if($gameMap._events[$gameVariables.value(169)]){
            $enemyhp_X = $gameMap._events[$gameVariables.value(169)].screenX();
            $enemyhp_Y = $gameMap._events[$gameVariables.value(169)].screenY();
            }
            }
            //--
	    sprite.z = 500;
...
Game_CharacterBase.prototype.isSpriteTouch = function(x,y) {
	 ...
	 if (y < this.lmAbove()) {return false};	 cccc
	 return true;
};
