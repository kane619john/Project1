...
Game_Party.prototype.loadActorImages = function() {
    for (var i = 0; i < this.members().length; ++i) {
      ...
      if(actor._faceName != ""){
      	ImageManager.loadPicture(actor._faceName+actor._faceIndex);
      }
    }
};
...
Game_Party.prototype.initializeBattleMembers = function() {
 ...
 // BUG修改位置  if ($gamePlayer) $gamePlayer.refresh();
};
...
Game_Party.prototype.rangeActorSort = function(arr) {
  var len = arr.length;
...
Game_Party.prototype.rearrangeActors = function() {
    ...
    var battleArray = [];
    this._battleMembers = this.rangeActorSort(this._battleMembers);
    for (var i = 0; i < this._battleMembers.length; ++i) {
      var actorId = this._battleMembers[i];
      if (actorId === null) continue;
...
Window_PartyMenuCommand.prototype.addRevertCommand = function() {
    ...
    this.addCommand("行动时需要全员", 'revert');
};
...
Window_PartyMenuCommand.prototype.addCancelCommand = function() {
    ...
    this.addCommand("满足行动值要求", 'cancel', enabled);
};
...
Window_PartySelect.prototype.initialize = function(commandWindow) {
    ...
    var ww = Graphics.boxWidth - commandWindow.width+100;
...
Window_PartySelect.prototype.drawItem = function(index) {   
    ...
    index<$gameParty.maxBattleMembers() ? this.drawEmpty(rect):this.drawEmpty3(rect);
    }
...
Window_PartySelect.prototype.drawEmpty2 = function(rect) {
	//alert()
    var color = this.gaugeBackColor();
    this.changePaintOpacity(false);
    this.contents.fillRect(rect.x, rect.y, rect.width, rect.height, color);
...
Window_PartySelect.prototype.drawEmpty3 = function(rect) {
	//alert()
...
//    if (Yanfly.Param.PartyShowFace ) {
 ...
 //   }
 //   else if (Yanfly.Param.PartyShowCharacter) {
...
Window_PartyList.prototype.makeItemList = function() {
    ...
    if(this._data.length>20 && this._data.length % 5 !=0){//补空判定
    	var len = this._data.length % 5;
    	for (var i=0;i<len;i++) {
    		this._data.push(0);
    	}
    }
...
Window_PartyList.prototype.itemWidth = function() {
    return 122;
};
...
Window_PartyList.prototype.drawBasic = function(actor, rect) {
   ...
   // }
    ...
    } //else this.drawText(actor.name(), rect.x + ibw -40, rect.y, rect.width - ibw+50);
     if(actor.id != 1 && actor.hp < actor.mhp/4){
...
Window_PartyDetail.prototype.refresh = function() {
   ...
   // this.drawActorCharacter(this._actor,0,0);
...
Window_PartyDetail.prototype.drawActorBasicInfo = function() {
   ...
   //this.drawActorEXP(this._actor, xpad+215, y+45*2, width-50);
    this.drawActorSimpleStatus(this._actor, xpad, y, width);
  //  this.drawText(" 增强:"+this._actor._mage, 0,this.height-130, width+200, 'left');
};
Window_Base.prototype.drawActorSimpleStatus = function(actor, x, y, width) {
  ...
  //  this.drawActorIcons(actor, x, y + lineHeight * 2);
    ...
    this.drawActorLevel(actor, x2, y + lineHeight * 1);
...
BattleManager.startBattle = function() {
  ...
  //  $gameParty.BatteChange1();//战斗人员车轮战判定
    //else if ($gameSystem._consBat.index==1)
    //$gameParty.BatteChange1();//战斗人员车轮战判定
...
Scene_Party.prototype.createCommandWindow = function() {
    this._commandWindow = new Window_PartyMenuCommand(1040, 0);
...
Scene_Party.prototype.onPartyOk = function() {
	...
	if(index>=$gameParty.maxBattleMembers()){
		...
		if($gameParty.maxBattleMembers()>3)$gameParty._battleMembers[3] = 0;
		if($gameParty.maxBattleMembers()>4)$gameParty._battleMembers[4] = 0;
		if($gameParty.maxBattleMembers()>5)$gameParty._battleMembers[5] = 0;
		this.refreshWindows();
		this._partyWindow.activate();
	}else {
...
Scene_Party.prototype.onPersonalCancel = function() {
...
//判断指定多位英雄是否全部在队伍中
Game_Party.prototype.MembersCheckArr = function(arr) {
	var actor = null;
	for (var i=0;i<arr.length;i++) {
		actor = $gameActors.actor(arr[i]);
