...
(function() {
  ...
  Window_MapHpGauge.prototype.initialize = function(id) {
  	this.actor = $gameActors.actor($gameParty._battleMembers[id]);
    if (SceneManager.isNextScene(Scene_Battle)) {
      var x = gaugeWindowBattleX;
      var y = gaugeWindowBattleY+64*id;
    } else {
      var x = $gameSystem._mapHpGaugeWindowX != null ? $gameSystem._mapHpGaugeWindowX : gaugeWindowX;
      var y = $gameSystem._mapHpGaugeWindowY != null ? $gameSystem._mapHpGaugeWindowY+64*id : gaugeWindowY+64*id;
    }
    ...
    if (actor) {
      ...
      for (var i = 0; i < gauges.length; i++) {
        ...
        if (gauge.type === 'HP') {
          ...
          if (gaugeParam.param !== actor.tp) return true;
        } else if (gauge.type === 'LV') {
 ...
 //     }
  ...
  Window_MapHpGauge.prototype.drawActorHp = function(actor, x, y, width) {
    width = width || 186;
  ...
  Window_MapHpGauge.prototype.drawActorTp = function(actor, x, y, width) {
    width = width || 96;
    var color1 = this.tpGaugeColor1();
    var color2 = this.tpGaugeColor2();
    this.drawStaticGauge(x, y, width, actor.tpRate(), color1, color2, "tp");
 //   this.changeTextColor(this.systemColor());
  ...
  Window_MapHpGauge.prototype.refreshFace = function() {
    ...
    if (!this._needFaceRefresh) {
      var x = gaugeWindowWidth - 156 + faceOffsetX;
      var y = faceOffsetY+7;
      var height = Math.min(gaugeWindowHeight-7, 144);
      this.drawFace(actor.faceName(), actor.faceIndex(), x, y, 144,64);
    }
  };
  ...
  Scene_Base.prototype.createMapHpGaugeWindow = function() {
  	if($gameParty._battleMembers[0]){
    this._mapHpGaugeWindow = new Window_MapHpGauge(0);
    this.addChild(this._mapHpGaugeWindow);
   }
  	if($gameParty._battleMembers[1]){
    this._mapHpGaugeWindow2 = new Window_MapHpGauge(1);
    this.addChild(this._mapHpGaugeWindow2);
   }  	if($gameParty._battleMembers[2]){
    this._mapHpGaugeWindow3 = new Window_MapHpGauge(2);
    this.addChild(this._mapHpGaugeWindow3);
   }  	if($gameParty._battleMembers[3]){
    this._mapHpGaugeWindow4 = new Window_MapHpGauge(3);
    this.addChild(this._mapHpGaugeWindow4);
   }  	if($gameParty._battleMembers[4]){
    this._mapHpGaugeWindow5 = new Window_MapHpGauge(4);
    this.addChild(this._mapHpGaugeWindow5);
   }	if($gameParty._battleMembers[5]){
    this._mapHpGaugeWindow6 = new Window_MapHpGauge(5);
    this.addChild(this._mapHpGaugeWindow6);
   }
  };
  ...
  Scene_Map.prototype.terminate = function() {
    if (!SceneManager.isNextScene(Scene_Battle)) {
    	if(this._mapHpGaugeWindow)this._mapHpGaugeWindow.hide();
    	if(this._mapHpGaugeWindow2)this._mapHpGaugeWindow2.hide();
    	if(this._mapHpGaugeWindow3)this._mapHpGaugeWindow3.hide();
    	if(this._mapHpGaugeWindow4)this._mapHpGaugeWindow4.hide();
    	if(this._mapHpGaugeWindow5)this._mapHpGaugeWindow5.hide();
    }
    _Scene_Map_terminate.call(this);
  if(this._mapHpGaugeWindow)this.removeChild(this._mapHpGaugeWindow);
  if(this._mapHpGaugeWindow2)this.removeChild(this._mapHpGaugeWindow2);
  if(this._mapHpGaugeWindow3)this.removeChild(this._mapHpGaugeWindow3);
  if(this._mapHpGaugeWindow4)this.removeChild(this._mapHpGaugeWindow4);
  if(this._mapHpGaugeWindow5)this.removeChild(this._mapHpGaugeWindow5);
  };
