...
//  IMAGEMANAGER
//-----------------------------------------------------------------------------
ImageManager.loadMapTravelGraphic = function(filename) {
    return this.loadBitmap('img/maptravel/', filename,0, true);
};
...
Scene_MapTravel.prototype.createBackgrounds = function() {
	...
	for (var i = 0; i < bgs.length; i++) {
		...
		this._bgSprites[i].bitmap = ImageManager.loadPicture(bgs[i][0],null,'images/');
...
Scene_MapTravel.prototype.createMap = function() {
	...
	this.mapSprite.bitmap = ImageManager.loadPicture(filename,null,'images/');
...
Scene_MapTravel.prototype.createForegrounds = function() {		
	...
	for (var i = 0; i < fgs.length; i++) {
		...
		this._fgSprites[i].bitmap = ImageManager.loadPicture(fgs[i][0],null,'images/');
...
Sprite_MapTravelIcon.prototype.cacheBitmap = function() {
	this._pw = 0;
	this._ph = 0;
	this.bitmap = ImageManager.loadPicture(this.data().image);
};
...
Window_MapTravelList.prototype.itemHeight = function() {
    return 64;
};
